...
class Reefscape:
    ...
    def __init__(self):
        self.terrainJSON = {}
        pathFichierTerrain = os.path.join(
            os.path.dirname(__file__), r"..", NOM_FICHIER_TERRAIN
        )
        with open(pathFichierTerrain, "r") as fichier:
            self.terrainJSON = json.load(fichier)
            self.poses = {}
            isFromEditMode = (
                IS_FROM_EDIT_MODE_KEY in self.terrainJSON
            ) and self.terrainJSON[IS_FROM_EDIT_MODE_KEY]
            for cle in self.terrainJSON:
                ...
                if not isFromEditMode:
                    offsetTranslation = Translation2d(
                        ROBOT_HALF_LENGTH * rotation.cos(),
                        ROBOT_HALF_LENGTH * rotation.sin(),
                    )
                    offsetRotation = Rotation2d.fromDegrees(180)
                location = Translation2d(
                    float(locationJSON[0]) + offsetTranslation.x,
        ...
        with open(filename, "w") as fichier:
            ...
            for pose in self.poses:
                self.terrainJSON[pose] = {
                    "pos": [
                        self.poses[pose].translation().x,
                        self.poses[pose].translation().y,
                    ],
    ...
    def __getAllCoralStationSlides(self, numeroStation: int) -> List[Pose2d]:
        return self.__getAllPointsWithPrefix(
            self.__getPrefixeAllianceStr() + "_s" + str(numeroStation) + "_"
        )
